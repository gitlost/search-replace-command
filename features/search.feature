Feature: Do global search

  Scenario: Basic search
    Given a WP install

    When I run `wp search foo`
    Then STDOUT should contain:
      """
      guid
      """

    When I run `wp search foo --skip-columns=guid`
    Then STDOUT should not contain:
      """
      guid
      """

    When I run `wp search foo --include-columns=post_content`
    Then STDOUT should be a table containing rows:
    | Table    | Column       | Matches | Type |
    | wp_posts | post_content | 0       | SQL  |


  Scenario: Multisite search
    Given a WP multisite install
    And I run `wp site create --slug="foo" --title="foo" --email="foo@example.com"`
    And I run `wp search foo --network`
    Then STDOUT should be a table containing rows:
      | Table      | Column | Matches | Type |
  ...
  Scenario: Don't run on unregistered tables by default
    ...
    When I run `wp search foo --all-tables-with-prefix`
    Then STDOUT should contain:
      """
      wp_awesome
      """
  ...
  Scenario: Run on unregistered, unprefixed tables with --all-tables flag
    ...
    When I run `wp search foo --all-tables`
    Then STDOUT should contain:
      """
      awesome_table
      """
  ...
  Scenario: Run on all tables matching string with wildcard
    ...
    When I run `wp search bar wp_post\?`
    ...
    When I try `wp search foo wp_opt\*on`
    Then STDERR should be:
      """
      Error: Couldn't find any tables matching: wp_opt*on
      """

    When I run `wp search foo wp_opt\* wp_postme\*`
    Then STDOUT should be a table containing rows:
      | Table         | Column       | Matches | Type |
  ...
  Scenario: Quiet search (which doesn't make much sense...or does it?!)
    Given a WP install

    When I run `wp search foo --quiet`
    Then STDOUT should be empty

  Scenario: Verbose search
    Given a WP install
    And I run `wp post create --post_title='Search this text' --porcelain`
    And save STDOUT as {POSTID}

    When I run `wp search 'Search' --verbose`
    Then STDOUT should contain:
      """
    ...
    When I run `wp search 'Search' --verbose --precise`
    Then STDOUT should contain:
      """
      Checking: wp_posts.post_title
      1 rows affected
  ...
  Scenario: Regex search
    Given a WP install
    When I run `wp search '(Hello)\s(world)' --regex`
    Then STDOUT should contain:
      """
      wp_posts
      """
  ...
  Scenario: Search within theme mods
    Given a WP install
    ...
    When I run `wp search subdomain.example.com --no-recurse-objects`
    Then STDOUT should be a table containing rows:
      | Table      | Column       | Matches | Type       |
      | wp_options | option_value | 0       | PHP        |

    When I run `wp search subdomain.example.com`
    Then STDOUT should be a table containing rows:
      | Table      | Column       | Matches | Type       |
      | wp_options | option_value | 1       | PHP        |
  ...
  Scenario: Search with quoted strings
    Given a WP install
    ...
    When I run `wp search '<a href="http://apple.com">Apple</a>'`
    Then STDOUT should contain:
      """
      1 match.
      """

    When I run `wp search '<a href="http://googl.com">Google</a>'`
    Then STDOUT should contain:
      """
      0 matches.
      """
    ...
    When I run `wp post create --post_content='<a href="http://apple.com">Apple</a>' --porcelain`
    And I run `wp search '<a href="http://apple.com">Apple</a>'`
    Then STDOUT should contain:
      """
      2 matches.
      """

  Scenario: Search a table that has a multi-column primary key
    Given a WP install
    ...
    When I run `wp search bar wp_multicol`
    Then STDOUT should be a table containing rows:
      | Table       | Column | Matches | Type |
      | wp_multicol | name   | 1       | SQL  |
      | wp_multicol | value  | 1       | SQL  |

  Scenario: Large guid search
    Given a WP install
    And I run `wp option get siteurl`
    And save STDOUT as {SITEURL}
    And I run `wp post generate --count=20`

    When I run `wp search {SITEURL}`
    Then STDOUT should be a table containing rows:
      | Table    | Column | Matches | Type |
      | wp_posts | guid   | 22      | SQL  |

  Scenario Outline: Choose search method (PHP or MySQL/MariaDB) given proper flags or data.
    Given a WP install
    And I run `wp option get siteurl`
    And save STDOUT as {SITEURL}
    When I run `wp search <flags> {SITEURL}`
  ...
  Scenario: Search with a regex flag
    Given a WP install

    When I run `wp search 'EXAMPLE.com' wp_options --regex`
    Then STDOUT should be a table containing rows:
      | Table      | Column       | Matches | Type       |
      | wp_options | option_value | 0       | PHP        |
    ...
    When I run `wp search 'EXAMPLE.com' wp_options --regex --regex-flags=i`
    Then STDOUT should be a table containing rows:
      | Table      | Column       | Matches | Type       |
  ...
  Scenario: Search with export not allowed
    Given a WP install

    When I try `wp search foo --export`
    Then STDERR should be:
      """
      Error: You cannot search and --export at the same time.
      """
